...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    closestGhostDis = 999999
    mostlikelypos = None
    for liveghost in livingGhostPositionDistributions:
      currentmostlikelypos = liveghost.sortedKeys()[0]
      currentDis = self.distancer.getDistance(pacmanPosition, currentmostlikelypos)
      if currentDis < closestGhostDis:
        closestGhostDis = currentDis
        mostlikelypos = currentmostlikelypos

    #find the action that brings the pacman closer
    Disafteract = 999999
    resultaction = None
    for action in legal:
      successorPosition = Actions.getSuccessor(pacmanPosition, action)
      nextDis = self.distancer.getDistance(successorPosition, mostlikelypos)
      if nextDis < Disafteract:
        Disafteract = nextDis
        resultaction = action
