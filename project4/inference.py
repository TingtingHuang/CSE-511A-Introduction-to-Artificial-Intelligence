...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance ==None:
		for n in range(self.numParticles):
			self.particles[n] = self.getJailPosition()
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
		newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
		newSample = util.sample(newPosDist)
		newParticles.append(newSample)
    self.particles = newParticles
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
		...
		for j in range(self.numGhosts):
			choose.append(random.choice(self.legalPositions))
		self.particles.append(tuple(choose))
  ...
  def observeState(self, gameState):
    ...
    for i in range(self.numGhosts):
		if noisyDistances[i] == None:
			for particle_tuple_1 in self.particles:
				particle_list_1 = list(particle_tuple_1)
				particle_list_1[i] = self.getJailPosition(i)
   
    for particle_tuple_2 in self.particles:
		particle_list_2 = list(particle_tuple_2)
		weight =1.0
		for i in range(self.numGhosts):
			if noisyDistances[i] == None:
				weight = 1.0
			else: 
				trueDistance = util.manhattanDistance(particle_list_2[i],pacmanPosition)
				weight *= emissionModels[i][trueDistance]
		allPossible[tuple(particle_list_2)] += weight

    allzero = 0
    for particle_tuple_3 in self.particles:
		if allPossible[particle_tuple_3] <> 0:
			allzero = 1
			break
    if allzero ==0:
		self.initializeParticles()
